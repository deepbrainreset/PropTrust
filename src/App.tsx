import { Building2, CheckCircle2, Search, ShieldCheck, Star, Users, ArrowRight, MapPin, BedDouble, Bath, Ruler, Sparkles } from 'lucide-react'
...
function PropertyCard({p}:{p:Property}){
  return <article className="property-card">
    <div className="property-image" style={{backgroundImage:`url(${p.image})`}}>
      ...
      <span className="score-badge">Score {p.score}/100</span>
    </div>
    <div className="property-body">
      <div className="eyebrow">{p.operation}</div>
      <h3>{p.title}</h3>
      <div className="location"><MapPin size={15}/>{p.location}</div>
      <strong className="price">{p.price}</strong>
      <div className="facts"><span><BedDouble size={16}/>{p.rooms} amb.</span><span><Bath size={16}/>{p.baths}</span><span><Ruler size={16}/>{p.sqm} m²</span></div>
    </div>
  </article>
}
...
export function App(){
  return <div className="app-shell">
    <header className="site-header">
      <Logo/>
      <nav><a href="#propiedades">Propiedades</a><a href="#profesionales">Profesionales</a><a href="#como-funciona">Cómo funciona</a></nav>
      <div className="header-actions"><button className="ghost">Ingresar</button><button className="primary">Publicar gratis</button></div>
    </header>

    <main>
      <section className="hero">
        <div className="hero-copy">
          <span className="pill"><Sparkles size={15}/> Marketplace inmobiliario con reputación verificable</span>
          <h1>Encontrá una propiedad. <span>Conocé quién está detrás.</span></h1>
          <p>Propiedades, dueños e inmobiliarias con información clara, reputación auditable y herramientas para decidir mejor.</p>
          <div className="search-box">
            <select aria-label="Operación"><option>Comprar</option><option>Alquilar</option><option>Temporario</option></select>
            <input placeholder="Barrio, ciudad o dirección" />
            <button className="primary"><Search size={18}/>Buscar</button>
          </div>
          <div className="trust-row"><span><CheckCircle2 size={17}/>Publicación gratuita</span><span><ShieldCheck size={17}/>Identidad y matrícula verificables</span><span><Star size={17}/>Reseñas con operación verificada</span></div>
        </div>
        <div className="hero-panel">
          <div className="score-card"><div><small>Trust Score</small><strong>96</strong><span>/100</span></div><ShieldCheck size={38}/></div>
          <div className="mini-card"><div className="avatar"><Building2/></div><div><b>Estudio Demo</b><span>Profesional de demostración</span></div><div className="rating"><Star size={15}/>4,9</div></div>
          <div className="mini-card"><Users/><div><b>Marketplace inverso</b><span>Los profesionales pueden competir por captar tu propiedad.</span></div></div>
        </div>
      </section>
